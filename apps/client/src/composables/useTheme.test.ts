...
assert.match(appSource, /@use "@\/styles\/colors\.scss";/);
assert.match(appSource, /@use "@\/themes\/skins\.scss";/);
...
assert.doesNotMatch(useThemeSource, /if \(effectiveTheme\.value === "dark"\) return ""/);
assert.match(useThemeSource, /return `theme-palette-\$\{effectivePalette\.value\}`/);
assert.match(useThemeSource, /default:\s*"#fff"/);
assert.match(useThemeSource, /warm:\s*"#fbf4e5"/);
assert.match(useThemeSource, /dark:\s*\{\s*default:\s*"#111715",\s*warm:\s*"#1b1511",\s*olive:\s*"#151a14",\s*cool:\s*"#121923"/);
