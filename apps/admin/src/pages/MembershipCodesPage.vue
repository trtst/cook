...
const batchFilterOptions = computed(() => batches.value.map(item => ({
...
async function loadPage() {
  loading.value = true;
  try {
    await Promise.all([loadSkus(), loadBatches(), loadCodes()]);
  } finally {
    loading.value = false;
  }
}
...
<template>
  <section class="page-stack membership-page" v-loading="loading">
    <div class="toolbar-panel page-toolbar">
      <el-button type="primary" :icon="Plus" @click="openBatchDialog">新建批次</el-button>
      <el-button :icon="Refresh" @click="loadPage">刷新</el-button>
      <div class="toolbar-spacer" />
      <span class="page-note">固定 SKU 由服务端自动同步；后台只可基于这 5 个 SKU 发码，其他 SKU 不会到账。</span>
    </div>

    <div class="table-panel" v-loading="skuLoading">
    ...
    <div class="table-panel">
      ...
      <div class="toolbar-panel page-toolbar membership-filter">
        <el-select v-model="codeQuery.batchId" class="toolbar-select toolbar-select--wide" placeholder="全部批次" clearable>
