...
冰箱属于当前用户本人。库存批次允许只有“有库存”而没有精确数量或单位；这类记录用于表达“可能有，数量未记录”，不能被服务端静默当作足量，也不能因为数量未知阻止做饭。

采购完成只处理已购项并自动写入个人冰箱，数量、单位和到期时间可以为空，不默认生成到期日。计划完成、饭局结束和采购完成都不等于实际消耗；只有用户确认“这顿做完了”时，服务端才按固定菜谱版本和当前库存估算消耗。精确库存按可比较单位自动扣减，数量未知或“适量”只记录粗略结果，不要求用户逐项输入。
...
8. 系统菜谱导入、用户菜谱受控推荐、收藏统计、统一食材和单位库、个人食材及后台审核；菜谱/灵感不提供点赞能力。
...
6. 完成采购：清单状态、已购项自动记入个人冰箱和未购项保留必须同事务完成；采购入库不触发消耗。
7. 做饭完成：固定菜谱版本、责任菜、个人冰箱扣减、幂等结果和撤销记录必须同事务完成。
8. 成员加入：同时校验主理人邀请额度、接受人加入额度、邀请状态和并发版本。
9. 成员退出或移除：结束成员关系并保留必要历史参与事实，不触碰个人业务数据。
10. 会员升级：订单、支付回调和个人权益切换必须幂等且原子。
...
11. `FridgeItem` 始终按 `userId` 归属个人；批次可以多条，`exactQuantity` 和 `exactUnitId` 可为空以表达数量未知，精确扣减按可比较批次处理。
