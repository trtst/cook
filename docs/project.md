...
8. Free/Plus 权益、统一逻辑空间、图片压缩、历史版本、回收站、升级折算和到期超额只读。
...
2. 加入餐食计划：锁定当前 `RecipeContentVersion`，`MealPlanItem` 不引用可变饭搭子入口。
3. 购物项合并：按 `operationId` 去重，同一清单内按 `mergeGroupKey` 原子合并。
4. 点菜确认：关闭征集、汇总回应、生成计划必须同事务完成。
5. 结束购物：归档清单、迁移未购项、批量入库必须同事务完成。
6. 成员加入：校验唯一活跃饭搭子、成员上限、原空间冻结和邀请状态，不能产生部分成功。
7. 成员退出：结束成员关系、恢复原空间、生成迁出快照和保留参与关系必须保持一致。
8. 会员升级：补差订单、支付回调和权益切换必须幂等且原子。
