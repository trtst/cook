...
| `[x]` | 饭局详情 | 小程序详情改造、分享链接、掌勺认领/释放已接通，`verify:dining-event-flow` 已完成真实 API 联调，`HBuilderX` 小程序编译链已通过；另已于 Sunday, August 23, 2026 跑通 `/pages_meal/event/index` 官方 `mp-weixin` 分包入口基础 smoke，以及 `/pages_meal/detail/index` 真实饭局正文主状态自动化，已断言标题、`参与人`、`菜单`、`采购准备`、`做饭助手`、`待认领` 与真实菜谱标题。当前剩余仅是真机 / 微信开发者工具人工走查与后续协作扩展项，不再阻塞本轮完成标记 | `docs/plans/dining-event-detail-execution.md` |
...
| `[x]` | 登录弹窗验证码链路 | `POST /auth/code-send` 已补齐，`verify:login-modal-flow` 已于 Sunday, August 23, 2026 在本地 `3100` 实例真实跑通 `code-send(LOGIN/BIND_PHONE) -> code-login -> users/me -> admin app-config upload/clear -> public app-config`；同日 `/pages/me/index` 官方 `mp-weixin` 自动化也已断言“通知中心 -> 我的勋章”顺序、会员入口默认隐藏，以及未登录走勋章入口后登录弹窗 `wechat / phone` 主状态与固定文案；真机 / 微信开发者工具人工走查仍建议后续统一补齐 | `docs/plans/login-modal-code-login-execution.md` |
