...
`recipe.inspirationCategoryId` 使用系统灵感分类的稳定 ID。菜谱转 JSON 时，已确认分类写入下表 ID；无法确认时填写 `null`。JSON 不填写分类名称、临时编号或数据库自增占位值。
...
2. 分类名称与 ID 不一致、分类无法确认或来源没有足够依据时，填写 `null`，进入待人工选择；不得猜测。
...
1. 建议填写 3～6 个，最多 8 个；无可靠依据时填写 `[]`。去除首尾空格后不得重复。
2. 依据菜名、介绍、食材、步骤和提示提取；不得编造。
3. 不把一级分类或固定结构化属性当作关键词。例如“家常便饭”“晚餐”“主菜”“川湘菜”“重辣”分别属于分类或结构化标签。关键词用于前台展示、聚合和搜索；固定标签用于后台审核和规则筛选，两者不互相替代。
