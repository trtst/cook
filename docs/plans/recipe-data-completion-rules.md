...
4. JSON 导入发布的灵感菜谱，从后台维护的 100 个灵感用户池中随机分配归属用户；本规则的用户补全流程主要服务用户菜谱。
5. 正式菜谱用量保存互斥的 `EXACT(quantity + unitId)` 或 `FUZZY(text = "适量")`；“适量”不是系统单位。
...
2. 食材事实以结构化 `ingredientId + amount` 为准；`amount` 为精确或模糊两种互斥结构。
3. 精确结构的 `quantity` 必须是可校验的正数，单位必须来自当前可用系统单位；模糊结构只允许 `text = "适量"`。
4. 后台不得用推断值静默覆盖正文事实。
...
1. 能映射到系统食材且数量、单位可校验时，转为结构化精确用量。
2. 识别到明确模糊用量时，统一转为 `FUZZY(text = "适量")`；来源“少许 / 按需”不得原样进入新版本。
3. 能识别食材但既无法确认精确数量和单位，也无法确认模糊语义时，保留导入原文并标记 `NEEDS_FIX`。
4. 无法稳定识别食材时，保留导入原文并标记 `NEEDS_FIX`。

JSON 导入未匹配到的食材，创建或复用 `PENDING` 系统食材并进入“待审核食材”；食材审核为 `ACTIVE`、归并或重新匹配前，引用该食材的导入项不能发布。
...
营养分析不估算“适量”的克重；只要存在模糊用量，状态最高为 `ESTIMATED`，全部食材均不可计算时为 `INSUFFICIENT`。其他营养分析不足原因包括：
...
```json
{
  ...
  "ingredients": [
    ...
    },
    {
      ...
      "amount": { "kind": "EXACT", "quantity": "10", "unitId": "unit-ml" }
    },
    {
