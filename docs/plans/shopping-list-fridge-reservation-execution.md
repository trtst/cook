...
> 历史执行记录：不作为新的 V1 主路径。2026-09-24 起，购物清单不要求库存差额、库存预占或采购入库确认；本文仅保留已落地实现、历史数据和兼容读取依据。
...
| 最小 API | 已实现 | `check / fridge / complete / fridge-items` 的轻量回包与预占字段已落地 |
| 最小表与约束 | 已实现 | 购物项库存预占主事实、释放与结算约束已落地 |
| 三端实现 | 已实现 | 客户端、API、Prisma 与 migration 已按本执行单落地；后台无新增页面 |
| 真实验收 | 进行中 | `verify:shopping-fridge-flow` 已于 Sunday, August 23, 2026 在本地 `3100` 实例真实通过，当前覆盖 `from-recipe -> shopping-gap -> from-gap -> from-plan -> from-event-gap -> fridge apply -> complete`；另已于 Sunday, August 23, 2026 跑通 `/pages_pantry/gap/index` 官方 `mp-weixin` 真实登录正文态自动化，并继续真实点击 `加入清单`，已断言页顶主标题、已选采购清单卡、真实缺口食材、`涉及 1 场饭局 · 1 道菜`、来源餐次与 `加入清单` 按钮，且已通过真实接口回查确认目标采购清单写入 `EVENT` 来源食材；`/pages_pantry/list-detail/index` 真实登录态下的清单正文主状态自动化也已断言真实清单标题、`采购进度`、食材名、数量与 `查看来源`；剩余主要是购物清单/冰箱页面手验 |
...
当前代码已按本执行单落地库存预占主链路；剩余主要是页面手动验收与总清单状态回填。
