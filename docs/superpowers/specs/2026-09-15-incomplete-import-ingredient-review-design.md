...
3. 命中 `ACTIVE` 或 `PENDING` 时复用该记录；命中 `DISABLED` 时保持现有“不自动恢复、不自动新建重复项”的行为；完全未命中时创建 `PENDING` 食材。
...
3. 待审核记录通过或归并后，既有引用回写把未发布导入草稿切换到最终 `ACTIVE` 食材；拒绝后条目仍保持不可发布并显示字段错误。
...
3. 命中现有 `ACTIVE/PENDING` 食材时仅修复草稿引用；命中 `DISABLED` 时报告并跳过；无匹配时创建 `UNCLASSIFIED + PENDING` 食材，合法系统单位存在则记录，否则保持空单位。
...
3. 同名已下架食材：不自动恢复，不自动创建重复正式食材；导入条目保持未匹配并由回填/导入结果报告。
4. 回填遇到版本冲突：跳过该条目并报告，不覆盖正在编辑的数据。
5. migration 发现重复待审核名称：迁移失败并报告，先人工核对引用后再继续。

## 测试与验收

1. 后端先写失败测试，覆盖未知食材在 `unitId=null` 时创建 `PENDING`、有单位时沿用该单位、同名 `PENDING` 复用、同名 `ACTIVE` 复用、同名 `DISABLED` 不自动恢复。
2. 审核服务测试覆盖缺单位记录可列表、不可快捷通过、审核创建/归并时必须补单位、通过后写入单位并回写所有未发布导入引用。
3. migration 校验 Prisma schema、外键、Check 和待审名称唯一索引；对 `ACTIVE + null defaultUnitId` 的直接写入必须失败。
4. 回填脚本测试覆盖 dry-run 零写入、apply、重复 apply、合法单位保留、缺失单位为 null、已下架跳过和版本冲突。
5. Admin 测试覆盖待审核列表“待补充”、审核弹窗不预选分类/单位、待归类卡片空单位展示，以及导入页 `PENDING` 与“未匹配”的文案区分。
