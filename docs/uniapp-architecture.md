...
4. 三张入口卡的数据由独立首页接口返回，前端只负责固定布局、主题和交互，不在页面里写死标题、副标题和跳转目标。
5. 左侧大模块和右侧两张小模块都只消费最小入口字段：`title / subtitle / targetType / targetValue / artImageUrl / badgeText`；当前 `targetType` 只允许 `PAGE` 和 `WEB_VIEW`，外链必须以 `https://` 开头。接口中的 `artImageUrl` 只代表卡片内部视觉图区，不是整张成品海报。
6. 卡片的磨砂背景、主题字色、边框、圆角和点击态继续由前端根据当前皮肤渲染，不让接口返回样式值。
7. `action-dock` 继续使用固定四坑位，不改成动态排序或 `1 主 3 次`。固定顺序为 `我想吃 / 问大家 / 随机 / 缺什么`，但后台可按坑位做上架/下架；首页只展示当前已上架入口。
8. 四宫格不是 4 个普通导航按钮，而是 4 个强中心：`我想吃` 是个人提案中心，`问大家` 是家庭协商中心，`随机` 是决策引擎，`缺什么` 是执行落地中心。
9. 四宫格首页卡片至少要表达 4 层信息：入口名、一句话定位、当前最重要的一条状态、点入后的主动作；不允许只展示图标和标题。
10. 四宫格点入后的第一页必须是各自的场景首页，不是空白壳页、纯列表页或单纯工具页。
11. `home-entries` 继续只承担运营配置、跳转目标和卡片视觉区资源；四宫格运行时状态由各自 owner 页面和业务事实负责，不从 `home-entries` 扩成通用状态聚合接口。
