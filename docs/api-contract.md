...
当前规则补充：系统菜谱统一使用 `isInspiration = true` 且挂系统分类作为业务口径，不能仅以 `ownerId` 是否为空判断灵感菜谱；导入发布的 `ownerId` 从 100 人灵感用户池随机选择，后台直接创建的系统菜谱可保留空 owner。用户菜谱满足发布必填项即可发布，标签、营养、美食助理和完整度等派生结果由服务端按当前 `RecipeContentVersion` 持久化生成。正式菜谱用量只接受 `quantity + unitId`，不接受 `FUZZY`、`amount.kind`、`amount.text` 或 `fuzzyText`。
